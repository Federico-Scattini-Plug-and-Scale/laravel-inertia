                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <form @submit.prevent="submit" class="flex flex-col mb-4">
                            ...
                            <div class="mb-6">
                                <label class="text-lg font-semibold">{{ __('Status') }}</label>
                                <Multiselect 
                                    v-model="form.status" 
                                    :options="statuses"
                                    label="label"
                            ...
                            <div class="mb-6">
                                <label class="text-lg font-semibold">{{ __('Validity days') }}</label>
                                <input type="number" min="0" v-model="form.validity_days" class="sm:rounded-lg w-full mt-1">
                                <div v-if="errors.validity_days" class="text-red-500">{{ errors.validity_days }}</div>
                            </div>
                            <div class="mb-6">
                                <label class="text-lg font-semibold">{{ __('Published at') }}</label>
                                <input type="date" v-model="form.published_at" class="sm:rounded-lg w-full mt-1">
                                <div v-if="errors.published_at" class="text-red-500">{{ errors.published_at }}</div>
                            </div>
                            <div class="mb-6">
...
export default {
    ...
    setup (props) {
        const form = useForm({
            ...
            status: props.jobOffer.status,
            validity_days: props.jobOffer.validity_days,
            published_at: props.jobOffer.published_at_formatted
		})
    console.log(form.published_at)
		const markers = ref([])
        ...
        function submit() {
            Inertia.post(route(usePage().props.value.locale + '.admin.joboffers.update', [props.jobOffer]), form, {
